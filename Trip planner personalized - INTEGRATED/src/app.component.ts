...
export class AppComponent {
  ...
  async handlePlanRequest(planRequest: PlanRequest) {
    ...
    try {
      ...
      // Asynchronously generate the hero image after the itinerary is displayed
      this.generateAndLoadImages(result, planRequest.destination);
    } catch (err) {
      this.error.set('Sorry, we couldn\'t generate your itinerary. Please try again later.');
      this.view.set('error');
      console.error(err);
    }
  }

  async generateAndLoadImages(itinerary: Itinerary, destination: string) {
    // Generate Hero Image
    try {
      const heroPrompt = `A beautiful, vibrant, photorealistic image of ${destination}, India, capturing its unique essence. Cinematic lighting.`;
      const heroImageUrl = await this.geminiService.generateImage(heroPrompt, '16:9');
      this.itinerary.update(current => {
        if (!current) return null;
        return { ...current, heroImageUrl: heroImageUrl };
      });
    } catch (e) {
